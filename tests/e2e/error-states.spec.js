...
test('hides the comment composer when Jira is anonymous read-only', async ({extensionApp, optionsPage, servers}) => {
  const target = requireJiraTestTarget(test, servers, {requireAuth: false});
  test.skip(target.mode !== 'mock', 'Anonymous read-only composer behavior is deterministic in mocked mode only.');
  ...
  await hoverIssueKey(page, '#popup-key');
  await expect(page.locator('._JX_comment_input')).toHaveCount(0);
  await expect(page.locator('._JX_comment_save')).toHaveCount(0);
